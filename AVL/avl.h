...
avl_node * insertNode(avl_node * n, int value){
	if(n == NULL)
...
avl_node * deleteNode(avl_node * n, int value){
	if(n == NULL)
		return n;
	if(value < n->value)
...
avl_node * replaceNode(avl_node *root, int value1, int value2){
	root = deleteNode(root, value1);
	return insertNode(root, value2);
}
